                                    ...
                                    <div class="material-datatables">
                                        <table id="datatables" class="table table-striped table-no-bordered table-hover"
                                            ...
                                            <tr v-for="item in faqs" :key="item.id">
                                                ...
                                                <td><b>{{ (item.categoryfaq.name.length > 30 ? item.categoryfaq.name.substring(0,30)+ "..." : item.categoryfaq.name) | upText }}</b></td>
                                                <td>
    ...
    export default {
        ...
        methods: {
            ...
            deleteItem(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        this.form.delete('/dashboard/faqs/' + id).then(() => {
            ...
            activeItem(id) {
                //Start Progress bar
                this.$Progress.start();
                this.form.get('/dashboard/active_faqs/' + id).then(() => {
                    /** Alert notify bootstrapp **/
                    var notify = $.notify('<strong>Please wait a moment</strong> ...', {
                        allow_dismiss: false,
                        showProgressbar: true,
            ...
            disableItem(id) {
                //Start Progress bar
                this.$Progress.start();
                this.form.get('/dashboard/disable_faqs/' + id).then(() => {
                    /** Alert notify bootstrapp **/
                    var notify = $.notify('<strong>Please wait a moment</strong> ...', {
                        allow_dismiss: false,
                        showProgressbar: true,
