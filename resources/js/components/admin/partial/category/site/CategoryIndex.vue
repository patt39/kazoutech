                                        ...
                                        <table id="datatables" class="table table-striped table-no-bordered table-hover"
                                            ...
                                            <tr v-for="item in categories" :key="item.id">
                                                ...
                                                <td class="td-actions text-right">
                                                    ...
                                                    <router-link  :to="{ path: `/dashboard/categories/${item.id}/edit` }" class="btn btn-link  btn-success btn-round btn-just-icon" title="Edit">
        ...
        methods: {
            ...
            deleteItem(id) {
                Swal.fire({
                    ...
                    text: "Are you sure you want to delete this Category Tag?",
                    type: 'warning',
                    animation: false,
                    ...
                    if (result.value) {
                        //Start Progress bar
                        this.$Progress.start();
                        this.form.delete('/dashboard/categories/' + id).then(() => {
                            ...
                            setTimeout(function() {
                                notify.update({'type': 'success', 'message': '<strong>Category Tag deleted successfully.</strong>', 'progress': 75});
                            }, 2000);
                            /*** End alert ***/
            ...
            activeItem(id) {
                //Start Progress bar
                this.$Progress.start();
                this.form.get('/dashboard/active_categories/' + id).then(() => {
                    /** Alert notify bootstrapp **/
                    var notify = $.notify('<strong>Please wait a moment</strong> ...', {
                        allow_dismiss: false,
                        showProgressbar: true
                    ...
                    setTimeout(function() {
                        notify.update({'type': 'success', 'message': '<strong>Category activated successfully.</strong>', 'progress': 75});
                    }, 2000);
            ...
            disableItem(id) {
                //Start Progress bar
                this.$Progress.start();
                this.form.get('/dashboard/disable_categories/' + id).then(() => {
                    /** Alert notify bootstrapp **/
                    var notify = $.notify('<strong>Please wait a moment</strong> ...', {
                        allow_dismiss: false,
                        showProgressbar: true
